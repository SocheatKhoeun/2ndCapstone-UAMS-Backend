...
@router.get("/", response_model=VerificationsPage, status_code=status.HTTP_201_CREATED)
async def list_verifications(out=Depends(ctl.list_verifications)):
    return out

@router.get("/{global_id}", response_model=VerificationOut, status_code=status.HTTP_201_CREATED)
async def get_verification(global_id: str, out=Depends(ctl.get_verification)):
    return out

@router.patch("/{global_id}", response_model=VerificationOut, status_code=status.HTTP_201_CREATED)
async def update_verification(global_id: str, out=Depends(ctl.update_verification)):
    return out

@router.post("/{global_id}/status", response_model=VerificationOut, status_code=status.HTTP_201_CREATED)
